...
//float WireguardComponent::get_setup_priority() const { return setup_priority::WIFI; }

void WireguardComponent::setup() {
...
WireguardComponent::WireguardComponent() { global_wifi_component = this; }

bool WireguardComponent::has_ap() const { return !this->ap_.get_ssid().empty(); }
bool WireguardComponent::has_sta() const { return !this->sta_.empty(); }
void WireguardComponent::set_fast_connect(bool fast_connect) { this->fast_connect_ = fast_connect; }
IPAddress WireguardComponent::get_ip_address() {
  if (this->has_sta())
...
std::string WireguardComponent::get_use_address() const {
  if (this->use_address_.empty()) {
    return App.get_name() + ".local";
  }
  return this->use_address_;
}
void WireguardComponent::set_use_address(const std::string &use_address) { this->use_address_ = use_address; }
void WireguardComponent::setup_ap_config_() {
...
float WireguardComponent::get_loop_priority() const {
  return 10.0f;  // before other loop components
}
void WireguardComponent::set_ap(const WiFiAP &ap) { this->ap_ = ap; }
void WireguardComponent::add_sta(const WiFiAP &ap) { this->sta_.push_back(ap); }
void WireguardComponent::set_sta(const WiFiAP &ap) {
  this->sta_.clear();
  this->add_sta(ap);
}

void WireguardComponent::start_connecting(const WiFiAP &ap, bool two) {
...
void WireguardComponent::print_connect_params_() {
  uint8_t bssid[6] = {};
...
void WireguardComponent::start_scanning() {
  this->action_started_ = millis();
...
void WireguardComponent::check_scanning_finished() {
  if (!this->scan_done_) {
...
void WireguardComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "WiFi:");
  this->print_connect_params_();
}

void WireguardComponent::check_connecting_finished() {
...
void WireguardComponent::retry_connect() {
...
bool WireguardComponent::can_proceed() {
  if (this->has_ap() && !this->has_sta()) {
    return true;
  }
  return this->is_connected();
}
void WireguardComponent::set_reboot_timeout(uint32_t reboot_timeout) { this->reboot_timeout_ = reboot_timeout; }
bool WireguardComponent::is_connected() {
  return this->state_ == WIFI_COMPONENT_STATE_STA_CONNECTED && this->wifi_sta_status_() == WL_CONNECTED &&
         !this->error_from_callback_;
}
void WireguardComponent::set_power_save_mode(WiFiPowerSaveMode power_save) { this->power_save_ = power_save; }

std::string WireguardComponent::format_mac_addr(const uint8_t *mac) {
  char buf[20];
  sprintf(buf, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  return buf;
}
bool WireguardComponent::is_captive_portal_active_() {
#ifdef USE_CAPTIVE_PORTAL
...
WireguardComponent *global_wifi_component;
